...
#     print("Thop library not found. FLOPs calculation will be skipped.")
THOP_AVAILABLE = False # Force disable thop features

# --- Custom Activation Functions (Keep as is) ---
...
MODEL_DIR = "models_unet"
RESULT_DIR_BASE = "results"
RESULT_DIR = os.path.join(RESULT_DIR_BASE, "unet")
...
# --- Hyperparameters ---
IMG_PATH = "/dist_home/siju/AFS/Sample1.jpg" # correct this path for new image
BATCH_SIZE = 8
EPOCHS = 100
TARGET_SIZE = (256, 256)
NUM_SAMPLES = 100
...
# --- Advanced Blurring Techniques (as in the methedology) ---
class AdvancedBlur:
    @staticmethod
...
# --- Dataset Generation (as in methedology) ---
...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None, activation_fn=nn.ReLU()):
        super().__init__()
        if not isinstance(activation_fn, nn.Module): raise TypeError("activation_fn must be an instance")
        mid_channels = mid_channels or out_channels
        act1_channels, act2_channels = mid_channels, out_channels

        if isinstance(activation_fn, FReLU):
            if act1_channels <= 0 or act2_channels <= 0: raise ValueError(f"Invalid channels for FReLU: {act1_channels}, {act2_channels}")
            self.activation1 = FReLU(channels=act1_channels)
            self.activation2 = FReLU(channels=act2_channels)
        elif isinstance(activation_fn, PARAMETRIC_ACTIVATIONS):
            self.activation1 = copy.deepcopy(activation_fn)
            self.activation2 = copy.deepcopy(activation_fn)
        else:
            self.activation1 = activation_fn
            self.activation2 = activation_fn

        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, 3, 1, 1, bias=False), nn.BatchNorm2d(mid_channels), self.activation1,
            nn.Conv2d(mid_channels, out_channels, 3, 1, 1, bias=False), nn.BatchNorm2d(out_channels), self.activation2
        )
...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels, activation_fn):
        super().__init__()
        act_fn_instance = copy.deepcopy(activation_fn) if isinstance(activation_fn, PARAMETRIC_ACTIVATIONS) else activation_fn
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2), DoubleConv(in_channels, out_channels, activation_fn=act_fn_instance)
        )
...
class Up(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels, activation_fn, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = DoubleConv(skip_channels + out_channels, out_channels, activation_fn=act_fn_instance)
        else:
    ...
    def forward(self, x1, x2):
        x1 = self.up(x1)
        if self.bilinear: x1 = self.conv_reduce(x1)
...
class UNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=3, activation_fn=nn.ReLU(), bilinear=True):
        ...
        factor = 2 if bilinear else 1; bottleneck_channels = c5 // factor

        self.inc = DoubleConv(n_channels, c1, activation_fn=self.activation_fn_template) # CORRECTED
        ...
        self.down4 = Down(c4, bottleneck_channels, activation_fn=self.activation_fn_template)

        self.up1 = Up(bottleneck_channels, c4, c4 // factor, self.activation_fn_template, bilinear=bilinear)
        self.up2 = Up(c4 // factor, c3, c3 // factor, self.activation_fn_template, bilinear=bilinear)
        self.up3 = Up(c3 // factor, c2, c2 // factor, self.activation_fn_template, bilinear=bilinear)
        self.up4 = Up(c2 // factor, c1, c1, self.activation_fn_template, bilinear=bilinear)
...
lpips_model_global = None
lpips_model_failed = False
...
# --- Activation analyzer class definition---
class ActivationAnalyzer:
    ...
    def _pre_act_hook(self, name, module, inp):
        ...
        if inp is not None and isinstance(inp, torch.Tensor) and inp.nelement() > 0:
             self.pre_act_maps[name].append(inp[0].detach().cpu())

    def _post_act_hook(self, name, module, inp, out):
        if isinstance(out, tuple): out = out[0]
        if out is not None and isinstance(out, torch.Tensor) and out.nelement() > 0:
             self.post_act_maps[name].append(out[0].detach().cpu())

    def register_hooks(self):
        self.remove_hooks(); self.pre_act_maps.clear(); self.post_act_maps.clear()
        hook_count = 0
        for name, module in self.model.named_modules():
            if isinstance(module, (nn.ReLU, nn.LeakyReLU, nn.Sigmoid, nn.Tanh, nn.ELU, nn.SiLU, Mish, Swish, ESwish, Aria, GCU, Snake, FReLU)):
    ...
    def analyze_batch(self, input_batch):
        ...
        self.model.eval()
        with torch.no_grad():
            _ = self.model(input_batch.to(next(self.model.parameters()).device))
...
def save_feature_maps(pre_act, post_act, act_name, layer_name, channel=0, result_dir=RESULT_DIR):
    ...
    save_path = os.path.join(save_dir, f"unet_{act_name}_{layer_name_safe}_C{channel}.png")
...
def save_comparison_plot(blurred, restored, original, act_name, run_idx, metrics=None, result_dir=RESULT_DIR):
    ...
    if metrics:
        ...
        metric_str = "\n" + " | ".join(items)
    plt.figure(figsize=(18, 6)); plt.clf(); plt.suptitle(f"U-Net Run {run_idx+1}", fontsize=16)
    ...
    save_path = os.path.join(save_dir, f"unet_{act_name}_run{run_idx+1}_comparison.png")
    try: plt.savefig(save_path, bbox_inches='tight', dpi=150)
...
def plot_loss_curves(train_losses, val_losses, act_name, run_idx, result_dir=RESULT_DIR):
    ...
    plt.title(f'U-Net Loss Curves ({act_name} - Run {run_idx+1})', fontsize=14)
    ...
    save_path = os.path.join(save_dir, f"unet_{act_name}_run{run_idx+1}_loss_curves.png")
    try: plt.savefig(save_path, bbox_inches='tight', dpi=150)
...
def analyze_frequency_and_spatial(model, analyzer, gt_img, pred_img, blurred_img, act_name, run_idx, result_dir=RESULT_DIR):
    ...
    save_path = os.path.join(save_dir, f"unet_{act_name}_run{run_idx+1}_main_spectra.png")
    try: plt.savefig(save_path, bbox_inches='tight', dpi=150)
    ...
    # Pass the specific visualization batch's blurred image (needs batch dim and device)
    ...
    for layer_name in layers_to_visualize:
        ...
        if pre_map is not None and post_map is not None:
             ...
             for channel in range(min(1, num_channels)):
                 ...
                 # Pass single channel map C=1, H, W -> unsqueeze for B=1 -> [1, 1, H, W]
                 save_feature_maps(pre_map[channel].unsqueeze(0), post_map[channel].unsqueeze(0),
...
def objective(trial, act_name, act_config, hpo_train_loader, hpo_val_loader, device):
    ...
    model = UNet(activation_fn=copy.deepcopy(act_config["fn"])).to(device)
...
# --- Function to count zero FLOPs/Params for activations ---
def count_activation_flops_params(m, x, y):
    pass # Assign zero FLOPs and Params

# --- Training & evaluation function (with thop ) ---
def train_and_evaluate(act_name, act_config, learning_rate, weight_decay,
    ...
    print(f"\n=== Training U-Net Run {run_idx+1} with {act_name} (LR={learning_rate:.2e}) ===")
    model = UNet(activation_fn=copy.deepcopy(act_config["fn"])).to(device) # Use UNet
    ...
    # --- Evaluation (FLOPs, Time, Metrics) with Custom Ops for FLOPs AND Params ---
    flops = float('nan'); params_thop = float('nan'); inference_time_ms = float('nan')
    if THOP_AVAILABLE:
        try:
            dummy_input, _ = next(iter(test_loader)); dummy_input = dummy_input[:1].to(device)
            model_eval = model.to(device).eval()
            custom_ops_map = { # Map activation classes to the dummy counter
                nn.ReLU: count_activation_flops_params, nn.LeakyReLU: count_activation_flops_params,
                nn.Sigmoid: count_activation_flops_params, nn.Tanh: count_activation_flops_params,
                nn.ELU: count_activation_flops_params, nn.SiLU: count_activation_flops_params,
                Mish: count_activation_flops_params, Swish: count_activation_flops_params,
                ESwish: count_activation_flops_params, Aria: count_activation_flops_params,
                GCU: count_activation_flops_params, Snake: count_activation_flops_params,
                FReLU: count_activation_flops_params
            }
            macs, params_profile = profile(model_eval, inputs=(dummy_input, ), custom_ops=custom_ops_map, verbose=False)
            flops = macs * 2
            params_thop = params_profile
            print(f"FLOPs: {flops / 1e9:.2f} GFLOPs ({clever_format([macs, params_thop], '%.3f')})")
        except Exception as e:
            print(f"Warn: FLOPs/Params calc failed for {act_name}: {e}")

    # --- Inference time measurement (to console) ---
    try:
    ...
    vis_indices = list(range(min(BATCH_SIZE, len(test_loader.dataset))))
    # Make sure test_loader.dataset is used for subsetting
    vis_loader = DataLoader(Subset(test_dataset, vis_indices), batch_size=BATCH_SIZE) # Use test_dataset
    ...
    avg_final_metrics['flops_g'] = flops / 1e9 if not np.isnan(flops) else float('nan')
    ...
    if vis_sample_collected:
        ...
        # Pass the specific visualization batch's blurred image for analysis
        analyze_frequency_and_spatial(model, analyzer, vis_sharp, vis_output, vis_blurred, act_name, run_idx, result_dir=result_dir)

    print(f"--- U-Net {act_name} Run {run_idx+1} Final Avg Metrics ---")
...
if __name__ == "__main__":
    ...
    metrics_to_aggregate = ["psnr", "ssim", "epi", "hf_recon", "lpips", "final_train_loss", "final_val_loss", "flops_g", "inf_time_ms"]

    for run_idx in range(NUM_RUNS):
        print(f"\n========== STARTING U-NET RUN {run_idx+1}/{NUM_RUNS} ==========")
        seed = run_idx
        ...
        for act_name, act_config in ACTIVATIONS.items():
            ...
            if HPO_ENABLED and hpo_train_loader is not None and hpo_val_loader is not None:
                print(f"\n--- Optimizing LR for U-Net/{act_name} (Run {run_idx+1}) ---")
            ...
            elif not HPO_ENABLED: print(f"--- Skipping HPO for U-Net/{act_name}. Using default LR: {DEFAULT_LR:.2e} ---")
            else: print(f"--- Skipping HPO for U-Net/{act_name} due to invalid HPO loaders. Using default LR: {DEFAULT_LR:.2e} ---")
        ...
        print(f"========== COMPLETED U-NET RUN {run_idx+1}/{NUM_RUNS} ==========")
    ...
    print("\n=== Aggregated U-Net Activation Function Results (Mean ± Std Dev over Runs) ===")
    ...
    ranked_activations = sorted(valid_act_names_final, key=lambda x: aggregated_results[x].get(sort_key_metric, float('inf') if not reverse_sort else float('-inf')), reverse=reverse_sort) # Corrected default value
    ...
    results_csv_path = os.path.join(RESULT_DIR, "metrics", "unet_aggregated_results.csv")
    try:
        with open(results_csv_path, "w") as f: f.writelines(csv_lines)
        print(f"\nAggregated U-Net results saved to: {results_csv_path}")
    except IOError as e: print(f"\nError saving U-Net results to CSV: {e}")
    ...
    print("\n=== Rankings by other metrics (U-Net, using mean values) ===")
